...
namespace MOARANDROIDS
{
    ...
    {
        // Token: 0x06001265 RID: 4709 RVA: 0x0008C268 File Offset: 0x0008A668
        protected bool CheckSurgeryFailAndroid(Pawn surgeon, Pawn patient, List<Thing> ingredients, BodyPartRecord part, Bill bill)
        {
            float num = 0.75f;
            num *= surgeon.GetStatValue((!patient.RaceProps.IsMechanoid) ? StatDefOf.ResearchSpeed : StatDefOf.WorkSpeedGlobal, true);
            if (patient.InBed())
            {
                num *= patient.CurrentBed().GetStatValue(StatDefOf.WorkSpeedGlobal, true);
            }
            ...
            if (!Rand.Chance(num))
            {
                if (Rand.Chance(this.recipe.deathOnFailedSurgeryChance))
                {
                    ...
                    Messages.Message("MessageMedicalOperationFailureFatalAndroid".Translate(new object[]
                    {
                        surgeon.LabelShort,
                        patient.LabelShort,
                        this.recipe.label
                ...
                else if (Rand.Chance(0.5f))
                {
                    if (Rand.Chance(0.1f))
                    {
                        Messages.Message("MessageMedicalOperationFailureRidiculousAndroid".Translate(new object[]
                        {
                            surgeon.LabelShort,
                            patient.LabelShort
                        }), patient, MessageTypeDefOf.NegativeHealthEvent);
                    ...
                    {
                        Messages.Message("MessageMedicalOperationFailureCatastrophicAndroid".Translate(new object[]
                        {
                            surgeon.LabelShort,
                            patient.LabelShort
                        }), patient, MessageTypeDefOf.NegativeHealthEvent);
                ...
                {
                    Messages.Message("MessageMedicalOperationFailureMinorAndroid".Translate(new object[]
                    {
                        surgeon.LabelShort,
                        patient.LabelShort
                    }), patient, MessageTypeDefOf.NegativeHealthEvent);
